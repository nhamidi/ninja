...
public class Sender_multicast {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			System.err
					.println("Invalid destination port. (must be above 1024)");
			System.exit(-1);
		}
		...
		Recepteur_thread_multi reception_thread = new Recepteur_thread_multi(
				destPort + 1, destIP, nb_of_recv);
		 ...
		 * System.in.read(); } catch (IOException e2) { e2.printStackTrace(); }
		 */
		...
		while (reception_thread.get_status_end_loop()) {
			...
			for (int block = 0; block < no_blocks; block++) {
				...
				EncodingSymbol[] symbols = encoded_symbols[block]
						.getEncoding_symbols();
				...
				try {
					...
					for (int i = 0; i < no_symbols; i++) {
						...
						byte[] serialized_data_with_length = new byte[serialized_data.length
								+ 2*size_int];
						nb_packet++;
						...
						//	System.out.println("                            "+k);
							...
							System.arraycopy(byte_array, 0,
									serialized_data_with_length, 0, size_int);
							System.arraycopy(packet_id, 0,
									serialized_data_with_length, size_int, size_int);
							
							System.arraycopy(serialized_data, 0,
									serialized_data_with_length, size_int*2,
									serialized_data.length);
							DatagramPacket sendPacket = new DatagramPacket(
									serialized_data_with_length,
									serialized_data_with_length.length, destIP,
									destPort);
							...
							//k = k + (int) Math.round((float) reception_thread.get_number_of_packets()* redondance);
						...
						// if you're sending to the localhost, you probably
						// want
						// to give some time
						// between packets, otherwise you'll be overfilling
						// the
						// send buffers (too fast)
					}
				} catch (IOException e) {
					e.printStackTrace();
		...
		/*
		 * System.out.println("Overhead : " + total_overhead + " octets soit :"
		 * + total_overhead_pourcent + "%");
		 * System.out.println("Nombre total d’ACK : " +
		 * reception_thread.get_nb_ack());
		 * System.out.println("Delai total d’envoi au premie récepteur: " +
		 * reception_thread.get_time_1());
		 * System.out.println("Delai total d’envoi : " +
		 * reception_thread.get_time_2());
		 */

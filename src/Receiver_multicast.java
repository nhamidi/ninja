...
public class Receiver_multicast {
	...
	public static void main(String[] args) throws Exception {
		...
		byte[] first_packet_data = new byte[receivePacket_for_the_size
				.getData().length - size_int * 2];
		System.arraycopy(receivePacket_for_the_size.getData(), size_int * 2,
		...
		total_symbols = (int) Math.round((float) total_symbols * 1.00);
		...
		while (!successfulDecoding) {
			...
			try {
				...
				for (int recv = 0; recv < total_symbols; recv++) {
					...
					if (para_test == 5
							&& (nb_of_utils_packet - 2) >= compteur_utils_packet) {
					...
					*/
					...
					if (((nb_of_utils_packet - 4) < compteur_utils_packet)
							&& para_test == 6) {
						para_test++;
						flag = FLAG_PUSH;
						System.out
			...
			for (int sblock = 0; sblock < no_blocks; sblock++) {
				...
				try {
					...
					//nb_of_utils_packet = (int) Math.round((float) nb_packets_lost * redondance);
					nb_of_utils_packet =nb_packets_lost;
					successfulDecoding = false;
					if (para_test == 3) {
						Recepteur_thread_multi recep_thread = new Recepteur_thread_multi(
								src_port, sendIP, 1);
						recep_thread.start();
				...
				try {
					...
					while (true) {
						...
						if (para_test == 1) {
							...
							sending_thread.send_thing();
							System.out
									.println("           envoie ACK 1 mauvais 3");
							recep_thread.join();
							break;
						}
					...
					System.out.println("Nombre total d’ACK : " + number_of_ack);

					System.out.println("Delai total d’envoi du message : "
							+ time);
					break;

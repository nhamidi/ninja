...
do
		...
		case $i in
			...
			5 )	 var5=`echo $line-$var1 | bc -l`
				...
				echo "${redondance[$j]} 0 $var2 $var3 $var4 $var5" >> file-tempo.txt
				j=`echo $j+1 | bc -l`
				i=1;;
				
		esac	
	done < /home/tai/workspace/stage_pfe/bin/histo/time_division_recv_final.txt
...
gnuplot << EOF
	...
	plot for [COL=2:14] 'file-tempo.txt' using COL:xtic(1) 
	#
EOF
